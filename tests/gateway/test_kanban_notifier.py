...
class CardRecordingAdapter(RecordingAdapter):
    """RecordingAdapter that also supports Kanban decision cards.

    Separate from RecordingAdapter on purpose: the notifier routes
    needs_input/capability blocks to send_kanban_decision_card() whenever the
    adapter exposes it, so tests asserting the plain-text send() path must use
    an adapter that genuinely lacks the capability.
    """
