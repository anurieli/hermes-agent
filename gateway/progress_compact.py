...
def sanitize_task_label(text: str, max_len: int = DEFAULT_TASK_LABEL_MAX_LEN) -> str:
    """Collapse whitespace and truncate text for use as a task label."""
    if not text:
...
def truncate_log_lines(
    lines: Sequence[str], max_chars: int = DEFAULT_LOG_MAX_CHARS
) -> Tuple[List[str], int]:
    """Keep the newest lines that fit and return their omitted-line count."""
    if not lines:
        return [], 0
    kept: List[str] = []
    total = 0
...
def build_expandable_quote(lines: Sequence[str]) -> str:
    """Render lines as a Telegram MarkdownV2 expandable blockquote."""
    if not lines:
        return ""
    body = list(lines)
    if len(body) == 1:
...
def render_compact_card(
    ...
    max_log_chars: int = DEFAULT_LOG_MAX_CHARS,
) -> str:
    """Render a live card header with a tap-to-expand activity log."""
    ...
    header = "\n".join(
        [f"📋 {label}", action, f"_{action_count} action{plural}_"]
    )
    kept, omitted = truncate_log_lines(list(log_lines), max_log_chars)
    quote_lines = list(kept)
    if omitted:
        note = f"… {omitted} earlier action{'' if omitted == 1 else 's'} omitted"
        quote_lines.insert(0, note)
